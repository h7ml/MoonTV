  ...
  allChannels.sort((a, b) => {
    switch (sort) {
      ...
      case 'quality': {
        ...
        return (qualityOrder[b.quality || 'SD'] || 0) - (qualityOrder[a.quality || 'SD'] || 0);
      }
      default:
        return 0;
    }
...
async function handleSearch(searchParams: URLSearchParams): Promise<NextResponse> {
  ...
  const matchedChannels = allChannels.filter(channel => 
    ...
    (channel.tags && channel.tags.some(tag => tag.toLowerCase().includes(query)))
  );
  
  // Get unique categories from results
  const matchedCategories = Array.from(new Set(matchedChannels.map(ch => ch.category)));
...
async function handleGetAll() {
  ...
  const stats = {
    totalChannels: categories.reduce((sum, cat) => sum + cat.channels.length, 0),
